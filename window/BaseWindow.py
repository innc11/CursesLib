...
class BaseWindow(ComponentContainer, ABC):
	...
	def __init__(self, monopolyMode, maskMode):
		...
		self.trbl = (0, 0, 0, 0)  # trbl是一个简易Resize回调,表明四个边的坐标的偏移量(重新初始化时生效),(0,0,0,0)就是铺满父窗口,四边没有空白,正数向内,不允许为负数,如需高级操作请重写onResize()
	...
	def addWindow(self, win, reinitialize=True):
		...
		if not isinstance(win, BaseWindow):
	...
	def drawBorder(self):
		...
		self.screen.box(curses.ACS_VLINE, curses.ACS_HLINE)
	...
	def getSubWinXywh(self, win):
		"""以本窗口为参考点，计算子窗口大小和位置(xywh)
		...
		cbr = win.onResize(width, height)  # 回调方式
		t, r, b, l = win.trbl  # 简易方式
		...
		if cbr is not None:  # 两种方式任选其一,但onResize()优先级更高
			x, y, w, h = cbr
	...
	def initialize(self, newScreen):
		...
		assert self.isDestroyed, "The Window has not been destroyed yet and can not initialize"
	...
	def reinitializeSubWins(self):
		...
		for win in self.subWindows:
			if win.isDestroyed:
				...
				x, y, w, h = self.getSubWinXywh(win)
	...
	def update(self, deltaTime):
		...
		assert not self.isDestroyed, "The Window has been destroyed, can not pass the click event"
	...
	def mouseWheel(self, x, y, directionUp):  # x,y均为0,没法使用,只能依靠self.lastClick
		...
		assert not self.isDestroyed, "The Window has been destroyed, can not pass the mouse wheel event"
	...
	def draw(self, forceRefresh=False):
		if forceRefresh or self.refreshFlag:
			assert not self.isDestroyed, "The Window has been destroyed, can not draw a frame"
			...
			for win in self.subWindows:
				if win.x == 0 and win.y == 0 and win.width == self.width and win.height == self.height:
					fullScreen = True
